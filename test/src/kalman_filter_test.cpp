...
std::vector<Measurement> GenerateMeasurements(float meas_var, float process_var, float dt=1.0f, size_t count = 50) {
...
TEST(TmpAddTest, TestToDo)
{
  ...
  // Initial values
  Eigen::Vector2f init_state = Eigen::Vector2f::Zero(); // Position and velocity
  ...
  // Measurment function
  Eigen::MatrixXf measurement_function = Eigen::MatrixXf::Zero(state_dim, meas_dim);
  measurement_function(0,0) = 1.0f;
  kf.SetMeasurementFunctionMatrix(measurement_function);


  // Get measurements
  std::vector<Measurement> measurements = GenerateMeasurements(5.0f, 0.1f, 0.1f, 50);
